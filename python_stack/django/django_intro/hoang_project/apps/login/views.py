...
def registration(request):
    if request.method == 'POST':
        ...
        if len(errors) > 0:
            ...
            User.objects.create(first_name=request.session['first_name'], last_name=last_name, email=email, username=username, birthday= birthday, password=hash_pass.decode())
...
def login(request):
    if request.method == 'POST':
        ...
        if len(errors) > 0:
            ...
            request.session['first_name'] = usery.first_name
